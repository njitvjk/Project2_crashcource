...
while True:
    ...
    random_arrysq = np.square(random_arry)

    # Step 3: Print your numbers
    print("************************************")
    print("Generated Random array for you is:", random_arry)
    ...
    print("Enter square of the number one by one :")
    user_arry = np.array([])
    ...
    for i in range(4):
        ...
        # print(user_arry)
    user_arrysq = np.square(user_arry)
    ...
    boolean_arry = array_members(random_arrysq,user_arrysq)
    result = np.all(boolean_arry == True)
    if result:
        print('Congratulations !! All values match, you have guessed it right')
    else:
        print("Sorry! The values does not match. The expected value is:", random_arrysq)
